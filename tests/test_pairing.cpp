...
void testX25519KeysAndEcdh() {
    ...
    TEST_CHECK(aliceKeys.has_value());
    TEST_CHECK(bobKeys.has_value());
    TEST_CHECK(aliceKeys->privateKey.size() == 32);
    TEST_CHECK(aliceKeys->publicKey.size() == 32);
    TEST_CHECK(bobKeys->privateKey.size() == 32);
    TEST_CHECK(bobKeys->publicKey.size() == 32);
    TEST_CHECK(aliceKeys->publicKey != bobKeys->publicKey);
    ...
    TEST_CHECK(aliceShared.has_value());
    TEST_CHECK(bobShared.has_value());
    TEST_CHECK(aliceShared->size() == 32);
    TEST_CHECK(bobShared->size() == 32);
    TEST_CHECK(CryptoProvider::verifyHmacConstantTime(*aliceShared, *bobShared));

    // 3. Reject invalid key lengths
    QByteArray badKey(31, 0xAA);
    TEST_CHECK(!CryptoProvider::deriveSharedSecret(badKey, bobKeys->publicKey).has_value());
    TEST_CHECK(!CryptoProvider::deriveSharedSecret(aliceKeys->privateKey, badKey).has_value());
...
void testArgon2idKdf() {
    ...
    TEST_CHECK(key1.has_value());
    TEST_CHECK(key2.has_value());
    TEST_CHECK(key1->size() == 32);
    TEST_CHECK(CryptoProvider::verifyHmacConstantTime(*key1, *key2));

    // Different password produces different key
    auto diffKey = CryptoProvider::deriveArgon2idKey("DifferentPin1234", salt);
    TEST_CHECK(diffKey.has_value());
    TEST_CHECK(!CryptoProvider::verifyHmacConstantTime(*key1, *diffKey));

    // Empty password rejected
    TEST_CHECK(!CryptoProvider::deriveArgon2idKey("", salt).has_value());
...
void testHmacSha256() {
    ...
    TEST_CHECK(hmac.has_value());
    TEST_CHECK(hmac->size() == 32);

    // Constant time match
    TEST_CHECK(CryptoProvider::verifyHmacConstantTime(*hmac, *hmac));

    // Tampered message
    QByteArray tamperedMessage = message + "X";
    auto tamperedHmac = CryptoProvider::computeHmacSha256(key, tamperedMessage);
    TEST_CHECK(!CryptoProvider::verifyHmacConstantTime(*hmac, *tamperedHmac));
    ...
    TEST_CHECK(!CryptoProvider::verifyHmacConstantTime(*hmac, *tamperedKeyHmac));
...
void testTimestampAndReplayProtection() {
    ...
    TEST_CHECK(cache.isTimestampValid(now, now));
    TEST_CHECK(cache.isTimestampValid(now - 150, now));
    TEST_CHECK(cache.isTimestampValid(now + 150, now));
    TEST_CHECK(cache.isTimestampValid(now - 300, now));
    TEST_CHECK(cache.isTimestampValid(now + 300, now));

    TEST_CHECK(!cache.isTimestampValid(now - 301, now));
    TEST_CHECK(!cache.isTimestampValid(now + 301, now));
    TEST_CHECK(!cache.isTimestampValid(0, now));
    TEST_CHECK(!cache.isTimestampValid(-100, now));

    // 2. Replay detection
    QString sess1 = "sess_001";
    QByteArray hash1 = QCryptographicHash::hash("transcript_alice_bob", QCryptographicHash::Sha256);

    TEST_CHECK(!cache.isReplayed(sess1, hash1, now, now));
    ...
    TEST_CHECK(cache.isReplayed(sess2, hash1, now, now));
    ...
    TEST_CHECK(!cache.isReplayed(sess2, hash2, now, now));
    ...
    TEST_CHECK(cache.isReplayed(sess1, hash1, now, now + 400));
...
void testEndToEndPairingHandshake() {
    ...
    TEST_CHECK(requestOpt.has_value());
    TEST_CHECK(aliceSession.state() == PairingState::AwaitingResponse);
    TEST_CHECK(requestOpt->senderDeviceId == aliceId);
    TEST_CHECK(requestOpt->receiverDeviceId == bobId);
    TEST_CHECK(!requestOpt->authProofBase64.isEmpty());
    ...
    TEST_CHECK(responseOpt.has_value());
    TEST_CHECK(bobSession.state() == PairingState::Paired);
    TEST_CHECK(responseOpt->status == "accepted");
    TEST_CHECK(bobSession.result().has_value());

    // 3. Alice handles incoming response
    MeckChat::Core::AppConfig::instance().setDeviceId(aliceId);
    bool aliceAccepted = aliceSession.handleIncomingResponse(*responseOpt, sharedSecret);

    TEST_CHECK(aliceAccepted);
    TEST_CHECK(aliceSession.state() == PairingState::Paired);
    TEST_CHECK(aliceSession.result().has_value());
    ...
    TEST_CHECK(aliceSecret.size() == 32);
    TEST_CHECK(bobSecret.size() == 32);
    TEST_CHECK(CryptoProvider::verifyHmacConstantTime(aliceSecret, bobSecret));

    // Verify WireGuard parameters exchanged
    TEST_CHECK(aliceSession.result()->peerDeviceId == bobId);
    TEST_CHECK(aliceSession.result()->peerVirtualIp == "10.77.0.3");
    TEST_CHECK(bobSession.result()->peerDeviceId == aliceId);
    TEST_CHECK(bobSession.result()->peerVirtualIp == "10.77.0.2");
...
void testPairingFailureScenarios() {
    ...
    TEST_CHECK(req.has_value());

    MeckChat::Core::AppConfig::instance().setDeviceId(bobId);
    PairingSession bobSession;
    auto failResp = bobSession.handleIncomingRequest(*req, "WrongPassword456", "10.77.0.3");
    TEST_CHECK(!failResp.has_value());
    TEST_CHECK(bobSession.state() == PairingState::Failed);
    TEST_CHECK(bobSession.lastError() == PairingError::AuthFailed);
    ...
    TEST_CHECK(!expiredResp.has_value());
    TEST_CHECK(bobSession2.state() == PairingState::Failed);
    TEST_CHECK(bobSession2.lastError() == PairingError::TimestampExpired);
    ...
    TEST_CHECK(validResp.has_value());

    // Replay the exact same request again
    auto replayedResp = bobSession3.handleIncomingRequest(*req, "CorrectPassword123", "10.77.0.3");
    TEST_CHECK(!replayedResp.has_value());
    TEST_CHECK(bobSession3.state() == PairingState::Failed);
    TEST_CHECK(bobSession3.lastError() == PairingError::ReplayDetected);
